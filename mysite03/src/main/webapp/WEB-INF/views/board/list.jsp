		...
		<div id="content">
			<div id="board">
				<form id="search_form" action="" method="post">
					<input type="text" id="kwd" name="kwd" value="${m.kwd }"> <input
							...
							<td style="text-align: left; padding-left:${(vo.depth-1)*20 }px">
								<c:choose>
									...
									<c:when test="${vo.title ne '삭제된 글입니다.' }">
										<a
											href="${pageContext.servletContext.contextPath }/board/view?no=${vo.no}">
											${vo.title }</a>
									</c:when>
									<c:otherwise>
									${vo.title }
								...
								<c:when test="${vo.title ne '삭제된 글입니다.' }">
									...
									<c:choose>
										<c:when
											...
											<td><a
												href="${pageContext.servletContext.contextPath }/board/deletepost?no=${vo.no}"
												class="del"
					...
					<ul>
						<c:choose>
							<c:when test="${m.serach == 1}">
								<c:set var="search" value="&kwd2=${m.kwd }" />
							</c:when>
							<c:otherwise>
